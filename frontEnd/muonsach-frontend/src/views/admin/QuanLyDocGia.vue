            ...
            <tr v-for="docgia in filteredReaders" :key="docgia._id || docgia.maDocGia || docgia.MaDocGia">
              ...
              <td class="text-end pe-4">
                <div class="d-inline-flex gap-2">
                  <button class="btn btn-sm btn-outline-warning rounded-3 px-2.5 py-1" @click="openModalToEdit(docgia)">
                    <i class="fas fa-edit me-1"></i>Sửa
                  </button>
                  <button class="btn btn-sm btn-outline-danger rounded-3 px-2.5 py-1" @click="confirmDeleteDocGia(docgia)">
        ...
        <div class="modal-content rounded-4 border-0 shadow text-center p-3">
          <div class="modal-body">
            ...
            <div class="d-flex gap-2">
              ...
              <button type="button" class="btn btn-danger rounded-pill flex-fill fw-bold text-white" @click="executeDeleteDocGia">Xóa</button>
            </div>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    async fetchReaders() {
      try {
        ...
        // Xử lý im lặng khi load dữ liệu
      }
    },
